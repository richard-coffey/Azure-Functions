...
public static class SiteCounterCosmosDbFunction
{
    ...
    public static async Task Run([QueueTrigger("site-counter", Connection = "QueueStorageConnectionString")] CloudQueueMessage siteCounterMessage, ILogger log)
    {
        ...
        ItemResponse<SiteCounter> response = await container.ReadItemAsync<SiteCounter>(siteCounter.Id, new PartitionKey(siteCounter.Id));
        SiteCounter autogeneratedSiteCounter = response.Resource;

        // Update the counter in the autogenerated site counter object
        autogeneratedSiteCounter.Counter = siteCounter.Counter;

        // Update the site counter object in CosmosDB
        await container.UpsertItemAsync<SiteCounter>(autogeneratedSiteCounter, new PartitionKey(autogeneratedSiteCounter.Id));
    }
}
}
